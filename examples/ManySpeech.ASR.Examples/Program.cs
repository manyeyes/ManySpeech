...
namespace ManySpeech.ASR.Examples
{
    internal static partial class Program
    {
        ...
        private static Dictionary<string, string> _defaultOnlineModelName = new Dictionary<string, string>{
            { "ASR", "paraformer-large-zh-en-int8-onnx-online" }
        };
        private static Dictionary<string, string> _defaultOfflineModelName = new Dictionary<string, string>{
            { "ASR", "paraformer-seaco-large-zh-timestamp-int8-onnx-offline" } };
        ...
        private static async void ExecuteRecognizer(Dictionary<string, object> config)
        {
            ...
            string defaultOnlineModelName = _defaultOnlineModelName.GetValueOrDefault("ASR");
            string defaultOfflineModelName = _defaultOfflineModelName.GetValueOrDefault("ASR");
            ...
            try
            {
                ...
                if (recognizerType == "online")
                {
                    ...
                    OnlineASR.Recognize(methodType, modelName, modelAccuracy, threads, files, modelBasePath);
                }
        ...
        {
            int i = 0;
            OfflineASR.ResetRecognitionResultHandlers();
            OfflineASR.OnRecognitionResult += async result =>
            {
        ...
        {
            int i = 0;
            OnlineASR.ResetRecognitionResultHandlers();
            OnlineASR.OnRecognitionResult += async result =>
            {
                string? text = AEDEmojiHelper.ReplaceTagsWithEmojis(result.Text.Replace("> ", ">"));
                if (!string.IsNullOrEmpty(text))
                {
        ...
        {
            OnlineASR.ResetRecognitionCompletedHandlers();
            OnlineASR.OnRecognitionCompleted += (totalTime, totalDuration, processedCount, sample) =>
            {
        ...
        {
            OfflineASR.ResetRecognitionCompletedHandlers();
            OfflineASR.OnRecognitionCompleted += (totalTime, totalDuration, processedCount, sample) =>
            {
                double elapsedMilliseconds = totalTime.TotalMilliseconds;
                Console.WriteLine("Recognition elapsed milliseconds:{0}", elapsedMilliseconds.ToString());
                Console.WriteLine("Total duration milliseconds:{0}", totalDuration.TotalMilliseconds.ToString());
        ...
        private static void PrintUsage()
        {
            if (_lang.ToLower() == "zh")
            {
                ...
                Console.WriteLine("\n示例1:");
                Console.WriteLine("  ASR.Examples.exe -type online -method one -base /path/to/directory -model my-model -accuracy int8 -threads 2 -files /path/to/0.wav /path/to/1.wav");
                Console.WriteLine("\n示例2（使用默认method=one）:");
                Console.WriteLine($"  set {EnvRecognizerType}=online && set {EnvModelBasePath}=/path/to/directory && ASR.Examples.exe");
                Console.WriteLine($"\n*应用程序目录：{applicationBase}, 如果不指定-base, 请将下载的模型存放于此目录。");
                Console.WriteLine($"\n*附加说明：按2次回车，可根据提示操作：1.选择语言；2.选择识别器类型。");
            }
            else
            {
                Console.WriteLine("\nUsage Instructions: ASR.Examples.exe [parameters]");
