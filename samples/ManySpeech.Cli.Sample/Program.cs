...
namespace ManySpeech.Cli.Sample
{
    internal static partial class Program
    {
        ...
        private static void PrintUsage()
        {
            if (_lang.ToLower() == "zh")
            {
                Console.WriteLine("\n使用说明: ManySpeech.Cli.Sample.exe [参数]");
                ...
                Console.WriteLine("\n示例1:");
                Console.WriteLine("  ManySpeech.Cli.Sample.exe -type online -method one -base /path/to/directory -model my-model -accuracy int8 -threads 2 -files /path/to/0.wav /path/to/1.wav");
                Console.WriteLine("\n示例2（使用默认method=one）:");
                Console.WriteLine($"  set {EnvRecognizerType}=online && set {EnvModelBasePath}=/path/to/directory && ManySpeech.Cli.Sample.exe");
                Console.WriteLine($"\n*应用程序目录：{applicationBase}, 如果不指定-base, 请将下载的模型存放于此目录。");
                Console.WriteLine($"\n*附加说明：按2次回车，可根据提示操作：1.选择语言；2.选择识别器类型。");
            }
            else
            {
                Console.WriteLine("\nUsage Instructions: ManySpeech.Cli.Sample.exe [parameters]");
