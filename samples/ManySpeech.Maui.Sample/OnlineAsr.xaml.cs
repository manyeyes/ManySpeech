...
public partial class OnlineAsr : ContentPage, IDisposable
{
    ...
    // 2.搜索 paraformer online onnx 流式模型
    ...
    public OnlineAsr(IRecorder micCapture, RecognizerCategory recognizerCategory, string modelName)
    {
        InitializeComponent();
        CheckModels();
    ...
    #region callback    
    private async void SetOnlineRecognizerCallbackForResult(IRecognizer recognizer, string? recognizerType = "online")
    {
        if (recognizer == null)
        {
            return;
    ...
    private void SetOnlineRecognizerCallbackForCompleted(IRecognizer recognizer)
    {
