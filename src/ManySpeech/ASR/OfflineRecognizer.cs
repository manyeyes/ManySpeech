    ...
    public class OfflineRecognizer : IDisposable
    {
            ...
            if (offlineModel != null)
            {
                ...
                switch (model.Replace("-", "").Replace("_", ""))
                {
                    ...
                    case "sensevoicesmall":
                        _offlineProj = new OfflineSenseVoiceSmall(offlineModel);
                        break;
                    case "seacoparaformer":
                        _offlineProj = new OfflineSeacoParaformer(offlineModel);
                        break;
                    case "funasrnanoctc":
                        _offlineProj = new OfflineFunAsrNanoCtc(offlineModel);
                        break;
                    case "funasrnanollm":
                        _offlineProj = new OfflineFunAsrNanoLLM(offlineModel);
                        break;
                    case "dolphinasr":
                        _offlineProj = new OfflineDolphinAsr(offlineModel);
                        break;
                    default:
        ...
        private void ProcessStreams(List<OfflineStream> streams)
        {
            ...
            foreach (OfflineStream stream in streams)
            {
                ...
                offlineInputEntity = stream.GetDecodeChunk();
            ...
            try
            {
                _offlineProj.Infer(modelInputs, tokenIdsList, timestampsList, languages, regions);
                int streamIndex = 0;
                foreach (OfflineStream stream in streams)
                {
