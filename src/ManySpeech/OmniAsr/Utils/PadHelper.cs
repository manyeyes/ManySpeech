...
namespace ManySpeech.OmniAsr.Utils
{
    internal static class PadHelper
    {
        public static float[] PadSequence(List<OfflineInputEntity> offlineInputs)
        {
            List<float[]?> floats = offlineInputs.Where(x => x != null).Select(x => x.Speech).ToList();
            return PadSequence(floats);
        }
        public static float[] PadSequence(List<OfflineInputEntity> offlineInputs, int tailLen = 0)
        {
            List<float[]?> floats = offlineInputs.Where(x => x != null).Select(x => x.Speech).ToList();
...
#pragma warning restore CS8602 // 解引用可能出现空引用。 
        ...
        public static float[] PadSequence_unittest(List<OfflineInputEntity> offlineInputs)
        {
            int max_speech_length = offlineInputs.Max(x => x.SpeechLength);
            int speech_length = max_speech_length * offlineInputs.Count;
            float[] speech = new float[speech_length];
            // 填充极小值（可选择交替符号避免直流偏移）
            float epsilon = 1e-3f;
            for (int i = 0; i < speech.Length; i++)
            ...
            for (int i = 0; i < offlineInputs.Count; i++)
            {
                float[]? curr_speech = offlineInputs[i].Speech;
