...
using ManySpeech.WenetAsr2.Model;
using ManySpeech.WenetAsr2.Utils;
using Microsoft.ML;
...
namespace ManySpeech.WenetAsr2
{
    /// <summary>
    ...
    public class OnlineRecognizer
    {
        ...
        {
            _onlineModel = new OnlineModel(encoderFilePath, decoderFilePath, ctcFilePath, configFilePath: configFilePath, threadsNum: threadsNum);
        ...
        private EncoderOutputEntity EncoderProj(List<OnlineInputEntity> modelInputs, List<float[]> statesList, int offset)
        {
            ...
            foreach (var name in inputMeta.Keys)
            {
                ...
                if (name == "required_cache_size")
                {
                    ...
                    //var xx=tensor.Dimensions;
                    container.Add(NamedOnnxValue.CreateFromTensor(name, tensor));
                }
                if (name == "att_cache")
                {
                    float[] att_cache = statesList[0];
                    int required_cache_size =  att_cache.Length / customMetadata.Num_blocks / customMetadata.Head / (customMetadata.Output_size / customMetadata.Head * 2);//_onlineModel.Required_cache_size;//
                ...
                if (name == "cnn_cache")
                {
                    ...
                    int[] dim = new int[] { customMetadata.Num_blocks, 1, customMetadata.Output_size, customMetadata.Cnn_module_kernel - 1 };//inputMeta[name].Dimensions
