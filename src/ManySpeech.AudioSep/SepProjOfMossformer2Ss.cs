...
{
    internal class SepProjOfMossformer2Ss : ISepProj,IDisposable
    {
        ...
        public SepProjOfMossformer2Ss(SepModel sepModel)
        {
        ...
        public List<ModelOutputEntity> ModelProj(List<ModelInputEntity> modelInputs, List<float[]> statesList=null, int offset=0)
        {
            ...
            var result = WavFrontend.AudioNorm(padSequence.Select(x => (double)x).ToArray());
            padSequence = result.normalizedAudio.Select(x => (float)x).ToArray();
            ...
            try
            {
                ...
                if (encoderResults != null)
                {
                    ...
                    float rmsInput = AudioProcessing.CalculateRms(padSequence);
                    foreach (var encoderResult in encoderResultsArray) {
                        ...
                        float[] output0 = AudioProcessing.NormalizeSample(outputTensor.ToArray(), rmsInput);

                        // 去掉额外增加的尾部采样
                        int sampleRate = modelInputs[0].SampleRate;
                        float[] output = new float[(int)(padSequence.Length - sampleRate * 0.5f) / 1];
