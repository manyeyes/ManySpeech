...
# ManySpeech.TextPunc 使用指南

## 一、简介
ManySpeech.TextPunc 是一款采用 C# 编写的“文本标点预测”库，其底层借助 Microsoft.ML.OnnxRuntime 对 onnx 模型进行解码操作。该库在多方面展现出良好特性：
...
### （三）手动安装
打开 NuGet 包管理器界面，在搜索栏中输入「ManySpeech」，然后点击「安装」按钮即可完成安装操作。
...
## 六、其他说明
- **测试用例**：以 `ManySpeech.TextPunc.Examples` 作为测试用例，方便使用者参考和验证功能。
...
| fireredpunc-zh-en-onnx | 21128 | 中文、英文 | [modelscope](https://www.modelscope.cn/models/manyeyes/fireredpunc-zh-en-onnx "modelscope") |

## 八、模型介绍

### （一）CT-Transformer 模型
#### 用途
CT-Transformer 模型是由阿里巴巴达摩院开源的 Controllable Time-delay Transformer 模型，其核心用途在于针对语音识别模型输出的文本进行标点预测，提升文本的可读性。如需了解更详细的模型信息，可访问以下链接：https://www.modelscope.cn/models/damo/punc_ct-transformer_zh-cn-common-vocab272727-pytorch/summary
...
### （二）FireRedPunc 模型
FireRedPunc：基于 BERT 的中英文标点预测.如需了解更详细的模型信息，可访问以下链接：https://modelscope.cn/models/xukaituo/FireRedPunc
